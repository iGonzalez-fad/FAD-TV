...
<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>	
	<div class="progression-studios-skrn-slider-background" <?php if( get_post_meta($post->ID, 'progression_studios_slider_header_image', true) ): ?>style="background-image:url('<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_slider_header_image', true)); ?>')"<?php else: ?><?php if(has_post_thumbnail()): ?><?php $image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'progression-studios-video-header'); ?>style="background-image:url('<?php echo esc_attr($image[0]);?>')"<?php endif; ?><?php endif; ?>>
		...
		<div style="display:none;">
         ...
         <video id="SkrnLightbox-<?php the_ID(); ?>"  <?php if( get_post_meta($post->ID, 'progression_studios_optional_locally_hosted_mp4_button', true) ): ?>poster="<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_optional_locally_hosted_mp4_button', true)); ?>"<?php endif; ?> <?php if( get_post_meta($post->ID, 'progression_studios_custom_dimension', true) == 'on' ): ?>width="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_width', true)); ?>" height="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_height', true)); ?>"<?php else: ?>width="960" height="540"<?php endif; ?> data-youtube-id="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_slider_btn_link', true)); ?>">
         </video>
			<?php endif; ?>
			
			<?php if( get_post_meta($post->ID, 'progression_studios_slider_btn_target', true) == "vimeo_video" ): ?>
         <video id="SkrnLightbox-<?php the_ID(); ?>"  <?php if( get_post_meta($post->ID, 'progression_studios_optional_locally_hosted_mp4_button', true) ): ?>poster="<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_optional_locally_hosted_mp4_button', true)); ?>"<?php endif; ?> <?php if( get_post_meta($post->ID, 'progression_studios_custom_dimension', true) == 'on' ): ?>width="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_width', true)); ?>" height="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_height', true)); ?>"<?php else: ?>width="960" height="540"<?php endif; ?> data-vimeo-id="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_slider_btn_link', true)); ?>">
         </video>
			<?php endif; ?>
			
			<?php if( get_post_meta($post->ID, 'progression_studios_slider_btn_target', true) == "mp4_video" ): ?>
         <video id="SkrnLightbox-<?php the_ID(); ?>"  <?php if( get_post_meta($post->ID, 'progression_studios_optional_locally_hosted_mp4_button', true) ): ?>poster="<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_optional_locally_hosted_mp4_button', true)); ?>"<?php endif; ?> <?php if( get_post_meta($post->ID, 'progression_studios_custom_dimension', true) == 'on' ): ?>width="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_width', true)); ?>" height="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_height', true)); ?>"<?php else: ?>width="960" height="540"<?php endif; ?>>
