...
class Frontend_Forms_UI {
	...
	function display_shortcode() {
		...
		if ( 'admin_form' == $type || 'admin_template' == $type ){
			...
			foreach( array( 'form', 'template' ) as $content ){	
				...
				// Show shortcode
				$form_shortcode = sprintf( '<code>[%s %s="%d"]</code>', FEA_PREFIX, $content, $post_id );
	...
	function columns_content( $column, $post_id ) {
		...
		if ( 'shortcode' == $column ) {
			...
			// Show shortcode
			echo sprintf( '<code>[%s %s="%d"]</code>', FEA_PREFIX, $content, $post_id );
	...
	*/
	...
	function enqueue_admin_scripts() {
		...
		wp_enqueue_style( 'fea-form-builder', FEA_URL .  'assets/css/admin'.$min.'.css', array(), FEA_VERSION );
		wp_enqueue_script( 'fea-form-builder', FEA_URL . 'assets/js/admin' .$min. '.js', array( 'jquery', 'acf-field-group', 'acf-input' ), FEA_VERSION, true );
	...
	function admin_form_display( $content ){
		...
		if( $post->post_type == 'admin_template' ){
			$form_preview = true;
			$content = '['.FEA_PREFIX.' template="'.$post->ID.'"]';
		}
