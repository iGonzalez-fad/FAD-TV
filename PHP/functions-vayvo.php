...
function progression_studios_scripts() {
	...
	wp_enqueue_style(  'font-awesome-new', get_template_directory_uri() . '/inc/fonts/font-awesome-new/css/all.min.css', array( 'vayvo-progression-style' ), '1.0.0' );
	...
	if ( is_singular( 'video_skrn' ) || is_singular( 'episodes_skrn' )) {  wp_enqueue_script( 'aztec-progression-smartTab', get_template_directory_uri() . '/js/jquery.smartTab.min.js', array( 'jquery' ), '20120206', true ); wp_enqueue_script( 'afterglow', get_template_directory_uri() . '/js/afterglow.min.js', array( 'jquery' ), '20120206', true );}
...
function is_valid_email_domain($login, $email, $arm_errors ){
 ...
 if( $valid === false ){
 $arm_errors->add('arm_reg_error','El correo sólo puede ser @fad.unam.mx');
 }
}
