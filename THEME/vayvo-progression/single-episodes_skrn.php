...
<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div id="video-page-title-pro" <?php if( get_post_meta($post->ID, 'progression_studios_episode_image', true) ): ?>style="background-image:url('<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_episode_image', true)); ?>');"<?php else: ?><?php if(has_post_thumbnail()): ?><?php $image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'progression-studios-video-header'); ?>style="background-image:url('<?php echo esc_attr($image[0]);?>');"<?php endif; ?><?php endif; ?><?php if( get_post_meta($post->ID, 'progression_studios_episode_video_embed', true) ): ?> class="video-embedded-media-height-post"<?php endif; ?>>
		...
		<?php if( get_post_meta($post->ID, 'arm_is_paid_post', true) && !current_user_can('administrator') || post_password_required( get_the_ID() ) ): ?>
			...
			<?php if( is_user_logged_in() ): ?>
				...
				?>
			  		...
			  		<?php if( get_post_meta($post->ID, 'progression_studios_episode_video_mp4', true) || get_post_meta($post->ID, 'progression_studios_episode_youtube_video', true) || get_post_meta($post->ID, 'progression_studios_episode_vimeo_video', true) ): ?>
			  		<a class="video-page-title-play-button afterglow" href="#Video-Vayvo-Single"><i class="fa fa-play"></i></a>
			        <div style="display:none;">
			           <video id="Video-Vayvo-Single" <?php if( get_post_meta($post->ID, 'progression_studios_episode_video_poster', true) ): ?>poster="<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_episode_video_poster', true)); ?>"<?php endif; ?> <?php if( get_post_meta($post->ID, 'progression_studios_custom_dimension', true) == 'on' ): ?>width="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_width', true)); ?>" height="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_video_height', true)); ?>"<?php else: ?>width="960" height="540"<?php endif; ?> <?php if( get_post_meta($post->ID, 'progression_studios_episode_youtube_video', true)): ?>data-youtube-id="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_episode_youtube_video', true)); ?>"<?php endif; ?> <?php if( get_post_meta($post->ID, 'progression_studios_episode_vimeo_video', true)): ?>data-vimeo-id="<?php echo esc_attr( get_post_meta($post->ID, 'progression_studios_episode_vimeo_video', true)); ?>"<?php endif; ?>>
			  				 <?php if( get_post_meta($post->ID, 'progression_studios_episode_video_mp4', true)): ?><source src="<?php echo esc_url( get_post_meta($post->ID, 'progression_studios_episode_video_mp4', true)); ?>" type="video/mp4"><?php endif; ?>
			           </video>
			        </div>
			  		<?php else: ?>
			  			<?php if( get_post_meta($post->ID, 'progression_studios_episode_video_embed', true)  ): ?>
			  				<div id="vayvo-single-video-embed"><?php echo apply_filters('progression_studios_video_content_filter', get_post_meta($post->ID, 'progression_studios_episode_video_embed', true)); ?></div>
			  			<?php endif; ?>
		...
		<div class="width-container-pro">
			<div id="video-post-container">
				...
				<ul id="video-post-meta-list">
					...
					<?php 
						$terms = get_the_terms( $post->ID , 'episode-season' ); 
						if ( !empty( $terms ) ) :
							echo '<li id="video-post-meta-cat"><ul>';
						foreach ( $terms as $term ) {
							$term_link = get_term_link( $term, 'episode-season' );
					...
					<?php if( get_post_meta($post->ID, 'arm_is_paid_post', true) && !current_user_can('administrator') || post_password_required( get_the_ID() ) ): ?>
						...
						<?php if( is_user_logged_in() ): ?>
							...
							?>
								...
								<?php if( get_post_meta($post->ID, 'progression_studios_episode_video_mp4', true) || get_post_meta($post->ID, 'progression_studios_episode_youtube_video', true) || get_post_meta($post->ID, 'progression_studios_episode_vimeo_video', true) ): ?><a href="#Video-Vayvo-Single" id="video-post-play-text-btn" class="afterglow"><i class="fa fa-play-circle" aria-hidden="true"></i><?php esc_html_e( 'Play', 'vayvo-progression' ); ?></a><?php endif; ?>
			...
			<div id="video-post-sidebar">
				...
				<?php if( get_post_meta($post->ID, 'progression_studios_poster_image', true) ): ?>
					<div class="content-sidebar-image noselect<?php if( get_post_meta($post->ID, 'progression_studios_episode_video_embed', true) ): ?>  video-embedded-media-height-adjustment<?php endif; ?>">			
