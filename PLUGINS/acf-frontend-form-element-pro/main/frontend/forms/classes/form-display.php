	...
	class Display_Form {
		...
		public function render_submit_button( $form, $index, $hidden = false ){
			if( isset( $form['steps'] ) ){ 
				...
				?>
				<div <?php echo acf_frontend_esc_attrs( $submit_attrs ) ?>>
			<?php
		...
		public function render_fields( $form, $current_step, $element, $hidden = true, $current_fields = array() ){
					...
					switch ( $form_field['field_type'] ) {
						case 'ACF_field_groups':
							if( $form_field['dynamic_acf_fields'] ){
								...
								$acf_field_groups = acf_frontend_get_acf_field_choices( array( 'groups' => $form_field['field_groups_select'] ), 'key' );
							}
							if ( $acf_field_groups ) {
		...
		public function show_messages( $form ){
			if ( fea_instance()->is__premium_only() ) {
				if ( frontend_admin_edit_mode() && ! empty( $form['style_messages'] ) ) {
		...
		public function render_form( $form, $preview = false ) {
			...
			if( ! empty( $form['show_in_modal'] ) && empty( $form['page_builder'] ) ){ 
				...
				acf_hidden_input( array( 'name' => 'form_'.$form['id'], 'value' => acf_encrypt( json_encode( $form ) ) ) );
				return;
			}
			...
			if( isset( $step_tabs ) ){
				if( ! empty( $form['fields'] ) ){
					$form = $this->render_fields( $form, false, $element, false, $form['fields'] );
				}
