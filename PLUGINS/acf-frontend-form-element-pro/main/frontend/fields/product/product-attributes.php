...
<div <?php acf_esc_attr_e( $div ); ?>>
	...
	<?php
		$clones_class = frontend_admin_edit_mode() && empty($field['value']) ? 'clone_list' : 'clones';
	?>
...
<div <?php echo acf_esc_attr($div); ?>>
	...
	<div class="acf-fc-layout-handle" title="<?php _e('Drag to reorder',FEA_NS); ?>" data-name="collapse-layout"><?php echo $title; ?></div>
		...
		if( !empty($value) ) {
			...
			foreach( $value as $row ) {
				...
				if( $row['acf_fc_layout'] == 'custom_product_attributes' ) {
					...
					$attr_options = $row[$row['acf_fc_layout'].'_terms'];
					if( is_array( $attr_options ) )	$attr_options = array_map('intval', $attr_options );
					$locations = $row[$row['acf_fc_layout'].'_locations'];
				}
